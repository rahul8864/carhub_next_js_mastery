                ...
                <Transition.Child
                  ...
                  leaveTo='opacity-0 scale-95'>
                    <Dialog.Panel className="relative w-full max-w-lg max-h-[90vh] overflow-y-auto transform rounded-2xl bg-white text-left shadow-xl transition-all flex flex-col gap-5 p-6">
                      <button type='button' onClick={closeModal} className='absolute z-10 p-2 rounded-full top-2 right-2 w-fit bg-primary-blue-100'>
                      ...
                      <div className='flex flex-col flex-1 gap-3'>
                        ...
                        <div className='flex gap-3'>
                          {['29', '33', '13'].map(item => (
                            <div key={item} className='relative flex-1 w-full h-24 rounded-lg bg-primary-blue-100'>
                              <Image src={generateCarImageUrl(car, `${item}`)} alt="car" fill priority className='object-contain' />
                            </div>
                          ))}
                        </div>
                      </div>
